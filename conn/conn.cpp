...
int main(){
	...
	if(!mysql_real_connect(conn,"39.107.59.4","food","123456","food",33061,NULL,0)){
		std::cerr<<"error connecting to database:"<<mysql_error(conn)<<std::endl;
		return 1;
	}

	if(mysql_query(conn,"SELECT id, username, password, phone, created_at, updated_at FROM food.`user`;")){
